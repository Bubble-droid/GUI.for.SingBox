...
STABLE_VER := $(shell gh release list --repo SagerNet/sing-box --exclude-pre-releases --json tagName,isLatest --jq '.[] | select(.isLatest == true) | .tagName' | head -n 1 | sed 's/^v//')
ALPHA_VER := $(shell gh release list --repo SagerNet/sing-box --json tagName,isPrerelease --jq '.[] | select(.isPrerelease == true) | .tagName' | head -n 1 | sed 's/^v//')
