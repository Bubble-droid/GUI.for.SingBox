...
export const generateRoute = (
  ...
  const isInboundEnabled = (id: string) =>
    inbounds.find((v) => v.id === id)?.enable;
  ...
  return {
    rules: route.rules.flatMap((rule) => {
      ...
      if (
        rule.type === RouteRuleType.Inbound &&
        isInboundEnabled(rule.payload) !== true
      ) {
        return [];
      }
      const extra: Recordable = generateRuleItem(
        rule,
        route.rule_set,
        inbounds,
      );
    ...
    rule_set: route.rule_set.map((ruleset) => {
      ...
      if (ruleset.type === RouteRuleType.Inline) {
        ...
        extra.path =
          localRuleset?.path.replace(
            /^data\//v,
            `${ctx.appEnv.appDataPath}/`,
          ) ?? '';
