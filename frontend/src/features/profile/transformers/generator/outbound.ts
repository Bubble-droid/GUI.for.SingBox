...
export const generateOutbounds = async (
  ...
  for (const outbound of outbounds) {
    ...
    if (outbound.type === OutboundType.UrlTest) {
      ...
      _outbound.interval = outbound.interval as NonNullable<
        OutboundOf<'urltest'>['interval']
      >;
      _outbound.tolerance = outbound.tolerance;
    }
    if (
      outbound.type === OutboundType.Selector ||
      outbound.type === OutboundType.UrlTest
    ) {
      _outbound.interrupt_exist_connections =
        outbound.interrupt_exist_connections;
      _outbound.outbounds = [];
      ...
      for (const proxy of outbound.outbounds) {
        if (proxy.type === 'Built-in') {
          if (
            ([OutboundType.Direct, OutboundType.Block] as string[]).includes(
              proxy.id,
            )
          ) {
  ...
  result.push(
    ...proxiesSet,
    ...[...builtInProxiesSet].map((v) => ({ type: v, tag: v })),
  );
