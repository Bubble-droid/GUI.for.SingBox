...
export const generateDns = (
  ...
  const getDnsServer = (id: string) =>
    dns.servers.find((v) => v.id === id)?.tag;
  ...
  return {
    servers: dns.servers.flatMap((server) => {
      ...
      if (
        ...
        server.domain_resolver &&
          (extra['domain_resolver'] = getDnsServer(server.domain_resolver));
        if (
          (
            [
              DnsServerType.Tcp,
          ...
          server.server_port &&
            (extra['server_port'] = Number(server.server_port));
          extra['server'] = server.server;
          if (
            (
              [DnsServerType.Https, DnsServerType.H3] as DnsServerType[]
            ).includes(server.type)
          ) {
      ...
      switch (server.type) {
        case DnsServerType.Hosts: {
          ...
          extra['predefined'] = Object.fromEntries(
            Object.entries(server.predefined).map(([k, v]) => [
              k,
              v.split(','),
            ]),
          );
    ...
    rules: dns.rules.flatMap((rule) => {
      ...
      if (
        ...
        rule.payload.includes('__is_fake_ip')
      ) {
      ...
      if (
        (
          [DnsActionKind.Route, DnsActionKind.RouteOptions] as DnsActionKind[]
      ...
      if (
        (
          [
            DnsActionKind.RouteOptions,
            DnsActionKind.Predefined,
          ] as DnsActionKind[]
        ).includes(rule.action)
      ) {
