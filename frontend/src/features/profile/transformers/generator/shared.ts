...
export const generateOutboundTls = (
  tls: OutboundTlsFormData,
): OutboundTlsOptions | undefined => {
  if (!tls.enabled) {
    return undefined;
  }
...
export const generateRuleItem = (
  ...
  const extra: Recordable = {
    action: rule.action,
    invert: rule.invert ? true : undefined,
  };
  switch (rule.type) {
    ...
    default: {
      extra[rule.type] = rule.payload.split(',').map((val) => {
        if (
          rule.type === RouteRuleType.Port ||
          rule.type === RouteRuleType.SourcePort
        ) {
          return Number(val);
        }
        return val;
