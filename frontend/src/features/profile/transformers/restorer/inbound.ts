...
export const restoreInbounds = (
  ...
  inbounds.flatMap((raw) => {
    ...
    if (raw['type'] === InboundType.Tun) {
      ...
      inbound.tun = {
        ...
        route_exclude_address:
          raw['route_exclude_address'] ?? template.route_exclude_address,
    ...
    if (raw['type'] === InboundType.Direct) {
      ...
      inbound.direct = {
        listen: {
          ...
          tcp_multi_path:
            raw['tcp_multi_path'] ?? template.listen.tcp_multi_path,
    ...
    if (
      [InboundType.Mixed, InboundType.Http, InboundType.Socks].includes(
        raw['type'],
      )
    ) {
      ...
      inbound[
        raw['type'] as Exclude<
          InboundType,
          typeof InboundType.Tun | typeof InboundType.Direct
        >
      ] = {
        listen: {
          listen: raw['listen'] ?? template.listen.listen,
        ...
        users:
          raw['users']?.map(
            ...
            (user: any) => `${user.username}:${user.password}`,
          ) ?? template.users,
      };
    }
    return inbound;
