...
const restoreExperimental = (
  ...
  return {
    clash_api: {
      ...template.clash_api,
      ...clash_api,
      access_control_allow_origin: normalizeArray(
...
export const restoreProfile = (
  ...
  const RouteRuleSetIds = legacyBuildTagIdMapping(
    'ruleset-',
  ...
  return {
    ...
    route: {
      ...
      auto_detect_interface:
        ...
        template.route.auto_detect_interface,
      find_process: config.route?.find_process ?? template.route.find_process,
      default_interface:
        config.route?.default_interface ?? template.route.default_interface,
      final: OutboundsIds[config.route?.final ?? ''] ?? template.route.final,
      default_domain_resolver: {
        server:
          DnsServersIds[
            (
              config.route?.default_domain_resolver as
