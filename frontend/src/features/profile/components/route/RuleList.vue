...
const handleAddEnd = () => {
  if (ruleId === -1) {
    const index = model.value.findIndex(
      (v) => v.type === RouteRuleType.InsertionPoint,
    );
    if (index === -1) {
...
const isSupportPayload = computed(
  () => fields.value.type !== RouteRuleType.RuleSet,
);
...
const hasLost = (rule: RouteRuleItem) => {
  ...
  const hasMissingInbound = !inboundOptions.some(
    (v) => v.value === rule.payload,
  );
  const hasMissingOutbound = !outboundOptions.some(
    (v) => v.value === rule.outbound,
  );
...
const renderRule = (rule: RouteRuleItem) => {
  ...
  if (type === RouteRuleType.RuleSet) {
    ...
    _payload =
      inboundOptions.find((v) => v.value === rule.payload)?.label ??
      rule.payload;
  }
  if (invert) {
    _payload += ` (invert) `;
  ...
  if (outbound) {
    const proxy =
      outboundOptions.find((v) => v.value === outbound)?.label ?? outbound;
    children.push(proxy);
  }
...
<template>
  ...
  <div v-draggable="[model, DraggableOptions]">
    <Card v-for="(rule, index) in model" :key="rule.id" class="mb-2">
      <div
        v-if="rule.type === RouteRuleType.InsertionPoint"
    ...
    <Card class="mt-4 mb-16">
      ...
      <template v-else-if="fields.action === RouteActionKind.Reject">
        <div class="form-item">
          ...
          <Radio
            v-model="fields.outbound"
            :options="RouteRejectMethodOptions"
          />
        </div>
      </template>
      ...
      <template v-else-if="fields.action === RouteActionKind.Resolve">
        ...
        <div class="form-item">
          ...
          <Select
            ...
            :options="[
              { label: 'kernel.strategy.byDnsRules', value: '' },
              ...dnsServerOptions,
            ]"
          />
        </div>
