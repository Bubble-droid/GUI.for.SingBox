...
import { NetnsTypeOptions } from '@features/constant/options'
...
const model = defineModel<NetnsConfig[]>({ required: true })
const { t } = useI18n()
const [showEditModal] = useBool(false)

let editIndex = -1
const fields = ref<NetnsConfig>(createNetns(NetnsType.Default))
...
<template>
  ...
  >
    ...
    <NetnsDefault v-if="fields.type === NetnsType.Default" :model-value="fields.config" />
    <NetnsUnshare v-else-if="fields.type === NetnsType.Unshare" :model-value="fields.config" />
  </Modal>
</template>
