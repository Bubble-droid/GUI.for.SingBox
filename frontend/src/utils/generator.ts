...
const generateOutbounds = async (outbounds: App.Outbound[], ctx: GenerateContext) => {
  ...
  return {
    ...
    rule_set: route.rule_set.map((ruleset) => {
      ...
      if (ruleset.type === RouteRuleType.Inline) {
        ...
        extra['path'] = _ruleset?.path.replace(/^data\//u, `${ctx.appEnv.appDataPath}/`)
...
export const legacyGenerateConfig = async (
  ...
  if (enablePluginProcessing) {
    config = await ctx.onGenerate(config, originalProfile as any)
  }
