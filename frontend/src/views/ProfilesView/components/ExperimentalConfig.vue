...
import type { ComponentOption, ExperimentalProfile } from '@/types'

interface Props {
  outboundOptions: ComponentOption[]
  modeOptions: ComponentOption[]
}

defineProps<Props>()

const model = defineModel<ExperimentalProfile>({
  required: true,
})
...
<template>
  <div>
    ...
    <div class="form-item">
      {{ t('kernel.experimental.clash_api.default_mode.title') }}
      <Select v-model="model.clash_api.default_mode" :options="modeOptions" />
    </div>
    <div class="form-item">
